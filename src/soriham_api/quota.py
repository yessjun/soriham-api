...
**비워 두면 무제한** — 주인의 스캔 워크스페이스가 그렇다.

전사 시간은 처리 로그의 최근 30일 합으로 센다. 로그가 워크스페이스를 직접 물고
녹음 삭제를 살아남기 때문에, 올리고-전사하고-지우고를 반복해도 한도가 돌아오지 않는다.
"""
...
def check_minutes(usage: Usage, duration_sec: float | None) -> None:
    ...
    막으면, 주인의 백로그에서 ffprobe가 실패하는 파일이 통째로 거절된다.
